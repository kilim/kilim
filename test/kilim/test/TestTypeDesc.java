...
public class TestTypeDesc extends TestCase {
    ...
    public static String commonSuperType(String oa,String ob) {
        return TypeDesc.commonSuperType(KilimContext.DEFAULT.detector,oa,ob);
    }
    public static String mergeType(String a,String b) throws IncompatibleTypesException {
        return TypeDesc.mergeType(KilimContext.DEFAULT.detector,a,b);
    }
    ...
    public void testCommonSuperTypes() {
        ...
        assertSame(commonSuperType("Ljava/io/Serializable;", "Ljava/lang/Comparable;"),
                D_OBJECT);
        assertEquals(commonSuperType("Lkilim/BasicBlock;", 
        "Lkilim/BasicBlock;"), "Lkilim/BasicBlock;");
        assertSame(commonSuperType("[Z", "[Z"), D_ARRAY_BOOLEAN);
        
        // least upper bound of Field and Method is AccessibleObject
        assertEquals("Ljava/lang/reflect/AccessibleObject;", 
                commonSuperType("Ljava/lang/reflect/Field;", 
        ...
        assertEquals("Lkilim/test/ex/ExA;", 
                commonSuperType("Lkilim/test/ex/ExA;", "Lkilim/test/ex/ExD;"));
        
        assertEquals("Lkilim/test/ex/ExA;", 
                commonSuperType("Lkilim/test/ex/ExD;", "Lkilim/test/ex/ExA;"));
        
        assertEquals("Lkilim/test/ex/ExA;", 
                commonSuperType("Lkilim/test/ex/ExC;", "Lkilim/test/ex/ExD;"));
        
    }
    
    public void testArray() throws IncompatibleTypesException {
        assertSame(D_OBJECT, 
                mergeType("Lkilim/test/ex/ExC;", "[Z"));
        
        assertSame(D_OBJECT, 
                mergeType("[Z", "Lkilim/test/ex/ExC;"));
    }
    
    public void testNull() throws IncompatibleTypesException {
        assertSame(D_NULL, mergeType(D_NULL, D_NULL));
        assertSame(D_OBJECT, mergeType(D_OBJECT, D_NULL));
        assertSame(D_OBJECT, mergeType(D_NULL, D_OBJECT));
    }
    ...
    public void testMerge() throws IncompatibleTypesException {
        ...
        // Null + String => String
        assertSame(D_STRING, mergeType(D_NULL, D_STRING));
        
        // Null + X == X (order of D_NULL flipped this time)
        assertSame(D_ARRAY_DOUBLE, mergeType("[D", D_NULL));
        
        // primitive types should return the same
        assertEquals(D_DOUBLE, mergeType(D_DOUBLE, D_DOUBLE));
        
        // Array + Object -> Array
        assertSame(D_OBJECT, mergeType("[I", D_OBJECT));
        
        assertSame(D_OBJECT, mergeType("[I", "[D"));
        
        // common supertype of arrays
        assertEquals("[Ljava/lang/reflect/AccessibleObject;",
                mergeType("[Ljava/lang/reflect/Field;","[Ljava/lang/reflect/Method;"));
        
        // A inherits from B ==> merge(A[], B[]) = B[]
        assertEquals("[Ljava/lang/reflect/AccessibleObject;", 
                mergeType("[Ljava/lang/reflect/Method;", "[Ljava/lang/reflect/AccessibleObject;"));
        
        // A inherits from B ==> merge(A[], B[]) = A[]
        assertEquals("[Ljava/lang/reflect/AccessibleObject;", 
                mergeType("[Ljava/lang/reflect/AccessibleObject;", "[Ljava/lang/reflect/Method;"));
